...
// teacher class
$t = mysqli_query($conn,"SELECT class FROM teachers WHERE username='$username'");
$tc = mysqli_fetch_assoc($t);
$class = trim(strtoupper($tc['class']));

$q = mysqli_query($conn,
  "SELECT s.name, s.class, a.status
   FROM attendance a JOIN students s ON s.id=a.student_id
   WHERE a.student_id='$sid' AND a.date='$date'
     AND UPPER(TRIM(s.class))='$class'"
);
